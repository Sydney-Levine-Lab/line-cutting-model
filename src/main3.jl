...
# --------------------------------------------------------------------
# Level-0 step and policy construction
# --------------------------------------------------------------------

"""
    build_boltzmann_policies(domain, memoized_h, temperature, N)

Build a vector of Boltzmann policies, one per agent, all sharing the same
memoized heuristic and temperature parameter.
"""
function build_boltzmann_policies(domain::Domain,
                                  memoized_h,
                                  temperature::Float64,
                                  N::Int)
...
"""
    level0_step!(state, policies, domain, N)

One timestep of level-0 reasoning:

- For each agent in order:
    - project others to walls (`project_others_to_walls`)
    - choose an action via the Boltzmann policy
    - apply the action to update the shared state

Returns the new state.
"""
function level0_step!(state::State,
