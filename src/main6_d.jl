...
# Example: MAPS="yes_line_E,new_yes_8" julia main6.jl
...
const DEBUG_EVAL        = env_bool("DEBUG_EVAL", false)  # print depth>=2 candidate evaluations
...
function choose_forward_action(initial_state::State, k::Int,
    ...
    # Collect (action, value) pairs for debug printing.
    # Allocates only when DEBUG_EVAL is on.
    debug_pairs = DEBUG_EVAL ? Vector{Tuple{Any,Float64}}() : nothing

    for act in candidate_actions
        value = evaluate_action(initial_state, k, act, domain, policies,
                                 order, idx_in_order, lookahead_rounds,
                                 level, steps_to_go)
        DEBUG_EVAL && push!(debug_pairs, (act, Float64(value)))
        if value > best_value
            best_value = value
            best_action = act
        end
    ...
    if DEBUG_EVAL
        ...
        @printf("[DEBUG d>=2 main6] k=%d (idx=%d) at %s — %d candidates, lookahead=%d\n",
                k, idx_in_order, kpos, length(candidate_actions), lookahead_rounds)
        for (a, v) in debug_pairs
