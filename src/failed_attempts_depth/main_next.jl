...
"""
Predict where all other agents will be after each of `depth` rounds of L0 play.

Returns a Vector of position vectors: `predicted[d]` gives all agents' positions
after round d. `predicted[d][n]` is agent n's position after round d.

Agent k does NOT move in the prediction.
"""
function predict_position_sequence(state::State,
                                    k::Int,
                                    domain::Domain,
    ...
    current_positions = copy(observed_positions)
    ...
    for d in 1:depth
        for n in order
            ...
            current_positions[n] = (new_l0_state[XLOC[n]], new_l0_state[YLOC[n]])
        end
        sequence[d] = copy(current_positions)
    end
...
- depth=1: predict one round of others' moves, project walls at predicted
  positions, A* to pick the best action. (1 A* call.)

- depth=2+: predict D rounds of others' positions. For each candidate first
  action, mentally move k there, then evaluate the resulting position using
  A* with round-2 walls. Pick the first action with the best lookahead
  evaluation. (~5 A* calls for depth 2.)

  More precisely, at depth D:
  - Round-1 walls are "temporary": they constrain which actions are valid now
  - Round-D walls are "persistent": A* evaluates the post-move position
    against these final walls
  
  This means k avoids cells occupied now AND picks a direction that looks
  good given where others will be D steps from now.
"""
function choose_action_with_depth(state::State,
                                   k::Int,
                                   domain::Domain,
                                   policies::AbstractVector{<:BoltzmannPolicy},
                                   steps_to_go,
    ...
    pos_seq = predict_position_sequence(state, k, domain, policies, order,
                                         observed_positions, depth)

    if depth == 1
        # Simple case: project walls at round-1 predicted positions, use policy
        planning_state = project_others_to_walls_at(state, k, domain, pos_seq[1])
        return SymbolicPlanners.get_action(policies[k], planning_state)
    end

    # depth >= 2: evaluate each candidate first action
    # First, get candidate actions from the round-1 wall configuration
    round1_view = project_others_to_walls_at(state, k, domain, pos_seq[1])
    candidate_actions = collect(PDDL.available(domain, round1_view))

    if isempty(candidate_actions)
        # No valid actions — return whatever the policy would pick
        return SymbolicPlanners.get_action(policies[k], round1_view)
    end

    # For each candidate action: mentally move k, evaluate with final-round walls
    best_action = candidate_actions[1]
    best_value = -Inf
    final_positions = pos_seq[end]  # where others will be after D rounds

    k_current_pos = (state[XLOC[k]], state[YLOC[k]])
    ...
    for act in candidate_actions
        ...
        # Build planning view from new position with final-round walls
        final_positions_with_k = copy(final_positions)
        final_positions_with_k[k] = k_new_pos
        lookahead_view = build_single_agent_view(state, k, domain,
                                                  k_new_pos, final_positions)

        # Evaluate: how many steps to goal from this position?
        # Use the steps_to_go heuristic (negative = better, fewer steps)
        # CHANGED 03-24
        # value = -steps_to_go(domain, lookahead_view, goal)
        
        # CHANGED 03-24
        # Run a fresh A* from the lookahead position to estimate remaining steps
        planner = SymbolicPlanners.AStarPlanner(WaterCollectionHeuristic(), max_nodes=2^16)
...
and pick actions via their Boltzmann policy.

With REASONING_DEPTH=1, agents predict one round of L0 play and plan from
the predicted wall configuration.

With REASONING_DEPTH>=2, agents predict D rounds, then evaluate each candidate
first action against the final predicted wall configuration, picking the
action with the best lookahead.
...
function simulation_step(state::State,
                         ...
                         domain::Domain,
                         order::AbstractVector{Int},
                         agent_snapshots::Vector{Vector{Tuple{Int,Int}}},
                         steps_to_go)
    ...
    for idx in 1:N_AGENTS
        ...
        # --- Agent k plans from its own personal snapshot ---
        observed = copy(agent_snapshots[k])
        ...
        if REASONING_DEPTH == 0
            ...
            act = choose_action_with_depth(interim_state, k, domain, policies,
                                            steps_to_go, order, observed,
                                            REASONING_DEPTH)
        end
        ...
        # --- Update agent k's snapshot: what k sees right after moving ---
        agent_snapshots[k] = get_all_positions(interim_state)
    end

    return interim_state, agent_snapshots
end
...
function run_simulations()
    ...
    Threads.@threads for map_index in eachindex(MAP_FILES)
        ...
        for run in 1:RUNS
            ...
            # Reset current state
            state = initial_state
            # Each agent's initial snapshot is the starting positions
            start_positions = get_all_positions(state)
            agent_snapshots = [copy(start_positions) for _ in 1:N_AGENTS]
            ...
            elapsed_run = @elapsed begin
                if SAVE_TRAJECTORIES
                    ...
                    for t in 1:TIME_MAX
                        state, agent_snapshots = simulation_step(state, policies, domain,
                                                                 run_order, agent_snapshots,
                                                                 steps_to_go)
