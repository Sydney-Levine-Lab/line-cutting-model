...
T0_START=$SECONDS
ORDER=random REASONING_DEPTH=0 RUNS=5 VERBOSE=true RUN_LABEL=v2_d0 \
  julia +1.10 -t 8 --project=src src/main.jl
T0_ELAPSED=$(( SECONDS - T0_START ))

# Depth 1
echo "--- Starting depth 1 ---" | tee -a "$LOG"
T1_START=$SECONDS
ORDER=random REASONING_DEPTH=1 RUNS=5 VERBOSE=true RUN_LABEL=v2_d1 \
  julia +1.10 -t 8 --project=src src/main.jl
...
T2_START=$SECONDS
ORDER=random REASONING_DEPTH=2 RUNS=2 VERBOSE=true RUN_LABEL=v2_d2 \
  julia +1.10 -t 8 --project=src src/main.jl
