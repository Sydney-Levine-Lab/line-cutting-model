"
    Old version from Mid March 2026. 
    - Includes the possibility of running joe's model or doing similar stuff that is mostly technical
    (e.g., having fixed order; or agents who see where others went)
    - Has INFO_PROB to look at the minimum info provision that is needed to get joe-like results
    (which again is weird if we see the random order as a tie-breaking device for simultaneous actions and nothing more)
    - Has knobs for different planning algos
"
